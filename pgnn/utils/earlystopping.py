    ...
    def __init__(
        ...
        for stop_varname in stop_varnames:
            if stop_varname is StopVariable.LOSS:
                ...
                self.comp_ops.append(operator.lt)
                self.best_vals.append(np.inf)
            elif stop_varname is StopVariable.ACCURACY:
                self.stop_vars.append('acc')
                self.comp_ops.append(operator.gt)
                self.best_vals.append(-np.inf)
            elif stop_varname is StopVariable.OOD:
                self.stop_vars.append('ood')
                self.comp_ops.append(operator.gt)
                self.best_vals.append(-np.inf)
            elif stop_varname is StopVariable.OOD_WO_NETWORK:
                self.stop_vars.append('ood_wo_network')
                self.comp_ops.append(operator.gt)
                self.best_vals.append(-np.inf)
